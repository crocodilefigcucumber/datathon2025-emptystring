...
categorical_features = [
    ...
    "client_profile_country_of_domicile",
    "client_profile_nationality",
    "client_profile_gender",
    "client_profile_marital_status",
    "client_profile_inheritance_details_relationship",
    ...
    "client_profile_currency",
    "account_form_currency",
    "account_form_country_of_domicile",
]
date_features = [
    "passport_birth_date",
    "passport_passport_issue_date",
    "passport_passport_expiry_date",
    "client_profile_birth_date",
    "client_profile_passport_issue_date",
    "client_profile_passport_expiry_date",
    "client_profile_inheritance_details_inheritance_year",
]

date_features = [
    col for col in df_train.columns if ("date" in col.lower() or "year" in col.lower())
]
numerical_features = [
    "client_profile_aum_savings",
    "client_profile_aum_inheritance",
    "client_profile_aum_real_estate_value",
]
...
for base, col_dict in fields.items():
    if len(col_dict) > 1:
        ...
        df_train["inconsistencies_" + base] = df_train.apply(
            check_inconsistency, axis=1
        )
...
cols_to_drop = [
    col
    for col in df_train.columns
    if any(col.startswith(prefix) for prefix in doc_prfx)
]
...
# New cols
df_train["savings_inheratince_ratio"] = df_train["aum_savings"] / (
    df_train["aum_savings"] + df_train["aum_inheritance"]
)

df_train["savings_real_estate_ratio"] = df_train["aum_savings"] / (
    df_train["aum_savings"] + df_train["aum_real_estate_value"]
)

# Convert label_label to binary (e.g., 1 for "Reject", 0 for others)
if "label_label" in df_train.columns:
    df_train["is_rejected"] = (df_train["label_label"] == "Reject").astype(int)

# Compute correlations
if "is_rejected" in df_train.columns:
    correlation_inheritance = df_train[
        ["savings_inheratince_ratio", "savings_real_estate_ratio", "is_rejected"]
    ].corr()

    print("Correlation between inheritance ratios and rejection label:")
    print(correlation_inheritance["is_rejected"].drop("is_rejected"))
